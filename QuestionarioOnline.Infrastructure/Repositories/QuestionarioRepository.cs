...
public class QuestionarioRepository : IQuestionarioRepository
{
    ...
    public QuestionarioRepository(QuestionarioOnlineDbContext context)
    {
        _context = context;
    }
    ...
    {
        var query = _context.Questionarios
            .AsNoTracking()
            .Where(q => q.UsuarioId == usuarioId);
    ...
    public async Task<IEnumerable<Questionario>> ObterPublicadosAsync(CancellationToken cancellationToken = default)
    {
        return await _context.Questionarios
            .AsNoTracking()
            .Where(q => q.Status == Domain.Enums.StatusQuestionario.Ativo)
            .OrderByDescending(q => q.DataCriacao)
            .ToListAsync(cancellationToken);
    }
    ...
    {
        await _context.Questionarios.AddAsync(questionario, cancellationToken);
